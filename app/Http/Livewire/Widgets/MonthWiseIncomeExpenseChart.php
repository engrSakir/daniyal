...
class MonthWiseIncomeExpenseChart extends Component
{
    ...
    {
        $this->chart_id = 'MonthWiseIncomeExpenseChart';
        return view('livewire.widgets.month-wise-income-expense-chart');
    }
    ...
    {
        $begin = new DateTime(date('Y-1-01'));
        ...
        foreach ($period as $date) {
            array_push($data_set,[
                'date' => $date->format("F"),
                'income' => Invoice::whereMonth('created_at',  $date->format("m"))->whereYear('created_at',  $date->format("Y"))->sum('paid_amount'),
